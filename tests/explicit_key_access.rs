...
#[test]
fn simple_key_explicit() {
    let t: HashMap<u64, i32> = hash_map! {
        7: 3,
        8: 4,
        9: 88
    };
    let ctx = Context::default();
    let q = parse_query(r#" .key(8) "#).unwrap();
    let result: HashSet<String> = ctx.exec(&q, &t).map(|v| v.to_string()).collect();
    assert_eq!(result, hash_set_from!["4"]);
...
fn simple_key_var() {
    ...
    let ctx = Context::default().with_var("x", 8);
    let q = parse_query(r#" .key($x) "#).unwrap();
    let result: HashSet<String> = ctx.exec(&q, &t).map(|v| v.to_string()).collect();
    assert_eq!(result, hash_set_from!["4"]);
}
